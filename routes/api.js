...
router.post("/mycart", (req, res) => {
    ...
    var query2 = `select sum(c.price) as total_ammount from cart c where 1 < (select p.quantity from product p where p.id = c.booking_id ) and  c.usernumber = '${req.body.number}' and c.status is null;`
    pool.query(query+query1+query2, (err, result) => {
      if (err) throw err;
      else if (result[0][0]) {
